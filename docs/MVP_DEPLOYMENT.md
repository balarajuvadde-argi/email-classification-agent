...
it makes existing stored Gmail grants unreadable. With `DATABASE_URL` configured, grants, policy,
run history, and report rows survive Render web-service restarts.

The Postgres MVP stores a recovery snapshot in `inbox_pilot_state` and readable derived rows in:

- `inbox_pilot_runs`
- `inbox_pilot_run_messages`
- `inbox_pilot_run_properties`

Use the derived tables for pgAdmin review and reporting. The snapshot table is still kept so the
application can restore the full tenant state safely after a restart.
